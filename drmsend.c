...
//	sockfd = socket(AF_UNIX, SOCK_STREAM, 0);
	...
	if (-1 == bind(sockfd, (const struct sockaddr *)&addr, sizeof(addr))) {
		MSG("Cannot bind unix socket to %s: %d",
			addr.sun_path, errno);
		goto cleanup;
	}
	...
	if (-1 == listen(sockfd, 1)) {
		MSG("Cannot listen on unix socket bound to %s: %d",
			addr.sun_path, errno);
		goto cleanup;
	}
	connfd = accept(sockfd, NULL, NULL);
	if (connfd < 0) {
		MSG("Cannot accept unix socket: %d", errno);
		goto cleanup;
	}
	...
	{
		uint32_t i;
		for(i = 0; i < OBS_DRMSEND_MAX_FRAMEBUFFERS; ++i)
			fb_fds[i] = -1;
	}
...
//	while(true)
	{
			...
			if (!drmfb) {
				...
				if (!drmfb->handle) {
					...
					if (ret != 0 || fb_fd == -1) {
						...
						printf("%d: %d %dx%d %d\n",fb_index, fb->fb_id, fb->width, fb->height, fb->pitch);
					}
				}
				drmModeFreeFB(drmfb);
			}
		...
		const ssize_t sent = sendmsg(connfd, &msg, 0);
